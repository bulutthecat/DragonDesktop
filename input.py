...
class InputHandler:  
    ...
    def _on_key_normal(self, event):  
        ...
        # NEW: Alt + Tab -> Cycle windows forward  
        if keysym == XK.string_to_keysym("Tab") and (event.state & X.Mod1Mask):  
            # Check if Shift is also pressed (reverse)  
    ...
    def _on_key_release(self, event):  
        ...
        # Detect Alt key release to end alt-tab cycle  
        # Alt_L and Alt_R keysyms  
    ...
    def _on_click(self, event):  
        ...
        if win_obj:  
            ...
            try:  
                ...
                # ============================================  
                # FIX ISSUE #2: LARGER RESIZE CORNER  
                # ============================================  
                # Scale threshold with zoom for consistent physical size  
                # Base size: 80px at 1.0 zoom  
        ...
        # 6. CLIENT WINDOW CLICK - Focus and Replay (FIXED!)  
        for win_obj in self.wm.windows.values():  
            if event.window.id == win_obj.client.id:  
                ...
                # Step 2: Replay the event so Firefox/etc actually receives the click  
                try:  
                    self.wm.d.allow_events(X.ReplayPointer, event.time)  
                    self.wm.d.sync()  # Force immediate processing  
